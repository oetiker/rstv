...
through its [`Context`](../api/rstv/view/struct.Context.html), and the event
...
- **any other view** addressed by [`ViewId`](../api/rstv/view/struct.ViewId.html)
...
[`Context`](../api/rstv/view/struct.Context.html) carries a single
...
brokers, …) each push one [`Deferred`](../api/rstv/view/enum.Deferred.html)
variant onto it. The view never sees the apply step.

After the dispatch returns, [`pump_once`](../api/rstv/app/struct.Program.html)
...
1. **Add a variant** to [`Deferred`](../api/rstv/view/enum.Deferred.html),
