...
The atom is a [`Cell`](../api/tvision-rs/screen/struct.Cell.html): one screen
position holding a grapheme cluster (its text), a colour
[`Style`](../api/tvision-rs/color/struct.Style.html), and two flags marking the lead
...
[`DrawBuffer`](../api/tvision-rs/screen/struct.DrawBuffer.html) — a single fixed-width
...
[`Buffer`](../api/tvision-rs/screen/struct.Buffer.html) — the screen-sized grid,
...
reference). [`Buffer::diff`](../api/tvision-rs/screen/struct.Buffer.html#method.diff)
...
The [`Renderer`](../api/tvision-rs/backend/struct.Renderer.html) owns the back/front
buffer pair and a boxed backend, and runs one frame per call to
[`render`](../api/tvision-rs/backend/struct.Renderer.html#method.render):
...
The terminal seam is the [`Backend`](../api/tvision-rs/backend/trait.Backend.html)
...
| Backend | Role |
| ------- | ---- |
| [`CrosstermBackend`](../api/tvision-rs/backend/struct.CrosstermBackend.html) | Production. Wraps crossterm; sets up raw mode, the alternate screen, and mouse capture, and restores the terminal on `Drop`. |
| [`HeadlessBackend`](../api/tvision-rs/backend/struct.HeadlessBackend.html) | Tests. An in-memory grid that never blocks: `poll_event` pops a queued event or returns immediately, so tests drive the loop deterministically. |
...
[`DrawCtx`](../api/tvision-rs/view/struct.DrawCtx.html) — the clipped, themed
...
[`ColorDepth`](../api/tvision-rs/backend/enum.ColorDepth.html): `TrueColor` passes
