...
[`View`](../api/tvision-rs/view/trait.View.html) trait, but it is called at the same
...
`TView` becomes the [`View`](../api/tvision-rs/view/trait.View.html) trait (a
...
[`ViewId`](../api/tvision-rs/view/struct.ViewId.html) handle. Each time, the modern
...
- [Inheritance → trait + composition](inheritance.md) — the `TView` hierarchy
  becomes the [`View`](../api/tvision-rs/view/trait.View.html) trait plus
  [`ViewState`](../api/tvision-rs/view/struct.ViewState.html) composition.
- [Pointers & infoPtr → handles](handles.md) — `TView*` becomes a
  [`ViewId`](../api/tvision-rs/view/struct.ViewId.html) resolved through a
  downward-borrowed [`Context`](../api/tvision-rs/view/struct.Context.html).
...
- [Palettes & glyphs → Theme/Role](theme.md) — the palette chain becomes a
  [`Theme`](../api/tvision-rs/theme/struct.Theme.html) keyed by semantic roles.
