...
- **Up and sideways are handles.** Every link that used to be a `TView*`
  ...
  broadcast subject — becomes a [`ViewId`](../api/rstv/view/struct.ViewId.html):
...
into the view's own [`ViewState`](../api/rstv/view/struct.ViewState.html). A
view therefore knows its own handle; ask for it with
[`id()`](../api/rstv/view/struct.ViewState.html#method.id), which returns
...
borrowed [`Context`](../api/rstv/view/struct.Context.html): the owner's size,
...
[`find_mut`](../api/rstv/view/trait.View.html#method.find_mut) tree-walk: a
...
[`Event::Broadcast`](../api/rstv/event/enum.Event.html) carrying a `command`
