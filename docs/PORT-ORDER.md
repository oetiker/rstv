...
| 81 ✅ | `TColorItem`/`TColorGroup`/`TColorIndex` | — | `colorsel.h` (inline), `sclrsel.cpp` | `dialog` (colordlg) | MECHANICAL | **✅** pure data tree in `src/dialog/colordlg.rs`. Collections→`Vec`: `ColorItem {name:String,index:u8}` (immutable palette index); `ColorGroup {name:String,index:u8,items:Vec<ColorItem>}` — **`index` is mutable focus state** (focused-item position; NOT a ctor param, defaults 0, set by row 85 `setGroupIndex`), distinct from `ColorItem::index`; `with_item` builder replaces C++ `operator+` (no `std::ops::Add`, no `ColorGroupList` newtype — groups = bare `Vec<ColorGroup>`). `ColorIndex {group_index:u8,color_index:Vec<u8>}` — C++ var-len struct `new uchar[numGroups+2]` (`[256]` is a sentinel); `colorSize`→`color_index.len()` via `color_size()`. Row-85/87 logic NOT pulled forward. D12 dropped. |
| 82 ✅ | `TColorSelector` | `TView` | `colorsel.cpp`, `sclrsel.cpp` | `dialog` (colordlg) | MECHANICAL | **✅** `ColorSelector` in `src/dialog/colordlg.rs` — first **raw-BIOS-color** widget (draws the 16 palette colors literally via `Style::new(Color::Bios(..),..)`, not theme `Role`s; `icon='\u{2588}'`/marker `'\u{25D8}'`, `c==0` marker forced to attr `0x70`). Draw loop `0..4`+`DrawCtx` clip = the faithful equivalent of C++ `i<=size.y`+`TDrawBuffer` clip (bg height-2 shows 0–7; fg height-4 shows 0–15; 3 snapshots). `colorChanged`→**payload-less broadcast** `COLOR_FOREGROUND/BACKGROUND_CHANGED {source=self}` (D4); consumers (83/84) resolve via new `color()` accessor + `as_any_mut` (the `FileList::focused_rec` precedent; NO D10 value/set_value — selector has no getData/setData). Inbound `cmColorSet` breadcrumbed inert `TODO(row 83)` (its `TColorDisplay` source doesn't exist yet). Mouse single-shot (drag `TODO(row 31)`); nav wrap arithmetic verbatim (underflow-safe). New cmds 71/72/73. |
