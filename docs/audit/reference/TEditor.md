...
| `BufLen` (field) | 422 | PORTED | OK | `Editor.buf_len: usize` (+ `buf_len()` accessor) | 2 | private field; public read-only accessor `buf_len()` documents "logical text length". Guide: chars between start of buffer and cursor — matches (logical length up to gap). |
...
| `CurPos` (field) | 422 | PORTED | OK | `Editor.cur_pos: Point` (+ `cur_pos()` accessor) | 2 | private field; public accessor. `(col, row)` display position of the cursor. Matches CurPos.X=col, CurPos.Y=line. |
| `CurPtr` (field) | 422 | PORTED | OK | `Editor.cur_ptr: usize` (+ `cur_ptr()` accessor) | 2 | private; public accessor "cursor logical offset". The gap sits physically at `cur_ptr`. Matches. |
...
| `Delta` (field) | 423 | PORTED | OK | `Editor.delta: Point` (+ `delta()` accessor) | 2 | private; public accessor "scroll offset (viewport top-left)". Delta.X leftmost col, Delta.Y topmost line. Matches. |
...
| `Limit` (field) | 423 | PORTED | OK | `Editor.limit: Point` (+ `limit()` accessor) | 2 | private; public accessor "(max line length, line count)". Limit.X longest line (fixed at `MAX_LINE_LENGTH`=256, see note), Limit.Y line count. **Note**: C++ tracks the true longest-line length; Rust pins `limit.x = MAX_LINE_LENGTH` (256) in `set_buf_len`/ctor rather than measuring, which matches magiblot's own behavior (`maxLineLength` constant; horizontal extent is a fixed max, not measured). OK. |
| `Modified` (field) | 423 | PORTED | OK | `Editor.modified: bool` (+ `modified()` accessor) | 2 | private; public accessor. Set in `insert_buffer` only when `!is_clipboard` (matches C++ `if (isClipboard()==False) modified=True`); cleared by `clear_modified` (save tail). Matches. |
...
| `ChangeBounds` (method) | 425 | PORTED | OK | `Editor::change_bounds` (impl `View::change_bounds`) | 2 | Sets bounds, clamps `delta` to the content extent, flags a full redraw; scroll-bar params republish on next flush. Matches "adjust Delta so text stays visible, redraw". |
...
| `InsertText` (method) | 426 | PORTED | OK | `Editor::insert_text(text, select_text, ctx)` (public, ctx-taking) + `insert_text_core` (ctx-free) | 2 | C++ `InsertText` copies Length bytes from Text, selecting if SelectText. Rust splits into `insert_text_core` (the ctx-free `insert_buffer` call) and the public `insert_text` that locks, inserts, tracks the cursor, and unlocks/flushes — the context-threading split (deviation, documented in the module seam). Faithful. |
...
| `Valid` (method) | 428 | PORTED | OK | `Editor::valid` (impl `View::valid`) | 2 | Ignores the command and returns `is_valid` (false only if buffer alloc failed). Matches. FileEditor overrides to run the modified-save Yes/No/Cancel prompt (TFileEditor::valid). |
