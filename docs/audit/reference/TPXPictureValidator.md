...
| `Error` (method) | 513 | PORTED | OK | `PXPictureValidator::error(&mut Context)` | 2 | Guide: "Displays a message box indicating an error in the picture format, displaying the string pointed to by `Pic`." C++: `messageBox(mfError\|mfOKButton, errorMsg, pic)`. Rust: `"Error in picture format.\n {pic}"` via `ctx.request_message_box`. Matches (quotes the mask). |
...
| `IsValid` (method) | 513 | PORTED | OK | `PXPictureValidator::is_valid(&self, s) -> bool` | 2 | Guide: returns `True` if `Pic` is nil OR `Picture` returns `prComplete` for `S` — i.e. no further input needed. C++: `picture(copy, False) == prComplete`. Rust: runs the engine on a copy with `auto_fill=false`, returns `result == PicResult::Complete` (no write-back). Matches (tests `pic_three_digits_is_valid`, `pic_optional_zip_plus_four`, `pic_comma_alternatives`, `pic_iteration_and_group`). |
