...
| `Init` (constructor) | 442 | PORTED | OK | `FilterValidator::new(valid_chars: impl Into<String>)` | 2 | Guide: sets `ValidChars` to `AValidChars`. Rust `new` stores the set. Matches. |
| `Load` (constructor) | 442 | NOT-PORTED | — | — | — | Stream constructor; `TStreamable` dropped (deviation D12). |
| `Error` (method) | 442 | PORTED | OK | `FilterValidator::error(&mut Context)` | 2 | Guide: "Displays a message box indicating the text string contains an invalid character." C++: `messageBox(mfError\|mfOKButton, errorMsg)`. Rust pops "Invalid character in input" via `ctx.request_message_box` (Error kind, OK-only). Matches behaviour; the literal string is tvision-rs's own (C++ `errorMsg` is a resource). |
| `IsValid` (method) | 442 | PORTED | OK | `FilterValidator::is_valid(&self, s) -> bool` | 2 | Guide: `True` iff all chars of `S` are in `ValidChars`. C++: `strspn(s, validChars) == strlen(s)` — note this makes the **empty string valid** (`strspn("")==0==strlen("")`). Rust: `s.chars().all(\|c\| valid_chars.contains(c))` — also true for `""` (vacuous all). Empty-input behaviour matches exactly (test `filter_accepts_empty_string`). |
| `IsValidInput` (method) | 442 | PORTED | OK | `FilterValidator::is_valid_input(&self, s: &mut String, _suppress_fill) -> bool` | 2 | Guide: same char-set check while typing; `SuppressFill` ignored (a filter never fills); does not mutate. C++ `isValidInput` body is identical to `isValid` (`strspn==strlen`). Rust delegates to `is_valid`, ignores `suppress_fill`, never mutates `s`. Matches exactly. |
