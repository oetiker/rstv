...
| `Init` (constructor) | 536 | PORTED | OK | `tv::StaticText::new(bounds: Rect, text: impl Into<String>) -> StaticText` | 2 | Guide: calls `TView::Init`, sets `Text` to `NewStr(AText)`, sets `GrowMode |= gfFixed`. Rust: sets `grow_mode.fixed = true`, stores text as String. Matches. "how/when to construct" could be expanded. |
...
| `GetText` (method) | 537 | EQUIVALENT | OK | `tv::StaticText::text() -> &str` | 2 | C++ `getText(char* S)` copies text into a caller-provided buffer, capped at 255 bytes. Rust `text()` returns a `&str` reference — no copy, no cap. Documented (what), but "why no buffer / no 255-byte cap" could be made explicit for C++ veterans. Known idiomatic mapping: buffer-out → reference return. |
