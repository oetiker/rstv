...
| `suspend` (virtual method) | (app.h) | EQUIVALENT | OK | `Backend::suspend` (`backend` trait method) | 2 | C++: calls `TSystemError::suspend`, `TEventQueue::suspend`, `TScreen::suspend`. Rust: `Backend::suspend()` encapsulates all terminal subsystem suspension behind the backend abstraction. Known idiomatic mapping: subsystem dispatch → backend trait method. (Out of scope for this sweep — `src/backend.rs` not in the target file list.) |
| `resume` (virtual method) | (app.h) | EQUIVALENT | OK | `Backend::resume` (`backend` trait method) | 2 | Symmetric with `suspend`. `Backend::resume()` restores the terminal. (Out of scope for this sweep — `src/backend.rs` not in the target file list.) |

## Summary

- PORTED: 7   EQUIVALENT: 3   NOT-PORTED: 0   MISSING: 0   UNSURE: 0
- SUSPECT: 0   |   doc<3 (public): 2 (`suspend`/`resume` → `Backend` trait, `src/backend.rs` out of scope for this sweep)   |   → concept: 0
- No SUSPECT entries remain. `Init`/`GetTileRect`/`WriteShellMsg` all raised to score 3. `Done`, `Cascade`, `DosShell`, `HandleEvent`, `Tile` are private (RAII/private fn) — N/A for public doc. The `suspend`/`resume` rows map to `Backend::suspend/resume` in a file outside the sweep scope (stays 2).
