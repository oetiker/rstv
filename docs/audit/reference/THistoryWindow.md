...
| `GetPalette` (method) | 458 | EQUIVALENT | OK | delegated to `Window` → `tv::theme::Role` family | deferred-theme | Roles live in `src/theme.rs` — deferred to the theme pass. No change to `src/theme.rs`. |
...
| `CHistoryWindow` palette (7 entries) | 458 | EQUIVALENT | OK | `Window` blue family + `HistoryViewer::LIST_ROLES` | deferred-theme | Roles live in `src/theme.rs` — deferred to the theme pass. No change to `src/theme.rs`. |

## Summary

- PORTED: 3   EQUIVALENT: 4   NOT-PORTED: 1   MISSING: 0   UNSURE: 0
- SUSPECT: 0   |   doc<3 (public): 0   |   → concept: 0
- Raised: `Init/new` → 3. Deferred (theme pass): `GetPalette`, `CHistoryWindow` palette (roles in `src/theme.rs`). Private/crate fields (`viewer_id`) and `pub(crate)` methods (`get_selection`, `InitViewer` inlined in `new`) are N/A for the public bar.
- Notable finding: `historyId` is listed as a THistoryWindow field in the guide but is not a field of the C++ class (the id lives only in the THistoryViewer); both C++ and Rust are consistent — it is not a gap. The most actionable documentation gap was `GetPalette`: the 7-slot `cpHistoryWindow` mapping to the Rust `Window` blue family + `HistoryViewer::LIST_ROLES` is implicit; this is deferred to the theme pass.
