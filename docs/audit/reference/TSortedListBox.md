...
| `getData` / `setData` / `dataSize` (inherited) | — | EQUIVALENT | SUSPECT | `SortedListBox::value()` (gather); **no `set_value_ctx`** (scatter) | 2 | C++ inherits `getData`/`setData`/`dataSize` from `TListBox` (`setData` calls `focusItem(p->selection)`). Rust `SortedListBox::value()` returns `FieldValue::Int(focused)` for gather, but `SortedListBox` does **not** implement `View::set_value_ctx` (only `ListBox` does, `list_box.rs:164`), so the default no-op runs: a dialog scatter to a sorted list box silently fails to focus the target item. Undocumented divergence — may be intentional (sorted selection is search-driven) but should be confirmed. |
