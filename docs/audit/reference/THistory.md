...
| `GetPalette` (method) | 456 | EQUIVALENT | OK | `tv::theme::Role::HistoryArrow` + `Role::HistorySides` via `ctx.style()` | deferred-theme | Roles live in `src/theme.rs` — deferred to the theme pass. No change to `src/theme.rs`. |
...
| `CHistory` palette (2 entries) | 457 | EQUIVALENT | OK | `tv::theme::Role::HistoryArrow`, `tv::theme::Role::HistorySides` | deferred-theme | Roles live in `src/theme.rs` — deferred to the theme pass. No change to `src/theme.rs`. |

## Summary

- PORTED: 3   EQUIVALENT: 6   NOT-PORTED: 2   MISSING: 0   UNSURE: 0
- SUSPECT: 0   |   doc<3 (public): 0   |   → concept: 0
- Raised: `Init/new` → 3. Deferred (theme pass): `GetPalette`, `CHistory` palette (roles in `src/theme.rs`). Private fields (`link`, `history_id`) and pump-inlined methods (`InitHistoryWindow`, `RecordHistory`) are N/A for the public bar.
