...
- Notable findings: `pattern` raised to 3 (added customisation guidance and double-width note). `Role::Background` palette chain is deferred — it lives in `src/theme.rs`, which is out of scope for this sweep (theme pass).
