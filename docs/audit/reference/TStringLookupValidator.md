...
| `Init` (constructor) | 551 | PORTED | OK | `tv::StringLookupValidator::new(strings: Vec<String>) -> Self` | 2 | C++: `Init(AStrings: PStringCollection)` — calls `TLookupValidator::Init`, sets `Strings`. Rust: `new(strings)` — takes owned `Vec<String>`. Streaming (`TLookupValidator::Init` super-call) dropped (D12). Rustdoc says what; missing "when to use (vs. other validators)". |
...
| `Error` (method) | 552 | PORTED | OK | `tv::StringLookupValidator` impl of `Validator::error` | 2 | C++ `error` calls `MessageBox` to display "not in list" dialog. Rust `error` calls `ctx.request_message_box("Input is not in list of valid strings", Error, ok-only, None, None)` — the async-modal-from-a-view seam. Functionally equivalent. Doc explains *what* but not *how* the async seam works (→ concept in module doc). |
...
| `NewStringList` (method) | 552 | PORTED | OK | `tv::StringLookupValidator::new_string_list(&mut self, strings: Vec<String>)` | 2 | C++: `newStringList(AStrings)` disposes of the old collection, then sets `Strings := AStrings`. Nil arg means "dispose without replacing". Rust: replaces `self.strings` with the new `Vec`; old `Vec` is dropped automatically. Nil-arg (dispose-without-replacing) has no direct analog — passing an empty `Vec` is the idiomatic equivalent. No doc note on the nil-arg divergence; worth a sentence. |
