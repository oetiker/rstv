...
- SUSPECT: 0   |   doc<3 (public): 2   |   → concept: 1
- Notable finding: All three NOT-PORTED entries (`overflow`, `xsputn`, `otstream`) are C++ `streambuf`/`ostream` plumbing with no equivalent in Rust's I/O model; their absence is correct and follows documented deviations D11/D12. No gaps. The `GetPalette` chain (D7 mapping to `Role::ScrollerNormal`) could use an explicit note in the `TextDevice` trait doc pointing to the palette chain concept page.
