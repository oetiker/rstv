...
| `putAttribute(indent, attr)` | C++ header | PORTED | OK | `tv::DrawBuffer::put_attribute(indent, style)` | 2 | In `drawbuf.h` (inline), not in the 1992 guide body but in the magiblot header. Sets the style of a single cell, keeping its char. Doc explains what it does; "when to prefer it over `move_char`" (for the change-only-style case) could be one sentence richer. |
| `putChar(indent, c)` | C++ header | PORTED | OK | `tv::DrawBuffer::put_char(indent, ch)` | 2 | Inline in `drawbuf.h`. Sets the char of a single cell, keeping its style. Same doc-score note as `put_attribute`. |
| `capacity` / `data` (protected fields) | C++ header | EQUIVALENT | OK | `DrawBuffer.data: Vec<Cell>` (private); `tv::DrawBuffer::capacity() -> usize` | 2 | Rust exposes capacity as a method, not a field. Data is private (idiomatic). Public `cells() -> &[Cell]` exposes a read-only view. |
