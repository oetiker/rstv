...
|---|---|---|---|---|---|---|
| `Init` (constructor) | 479 | PORTED | OK | `tv::MenuBar::new(bounds: Rect, menu: Menu) -> MenuBar` | 2 | Guide: calls `TMenuView.Init`, sets `GrowMode` to `gfGrowHiX`, sets `Options` to `ofPreProcess`. Rust: `MenuBar::new` sets `grow_mode.hi_x = true` and `options.pre_process = true` — exact match. Unit test `ctor_sets_grow_and_preprocess` verifies both flags. Doc explains what `new` does; lacks "when to prefer MenuBar vs custom view" guidance. |
...
| `CMenuView` palette | 479 | EQUIVALENT | OK | `MenuColors::resolve(ctx)` → `Role::Menu*` via `Theme` | 2 | Guide: "Menu bars, like all menu views, use the default palette CMenuView to map onto the 2nd through 7th entries in the standard application palette." Rust: `MenuBar::draw` resolves colours via `MenuColors::resolve` (same six semantic slots). Known idiomatic mapping: class Palette → `tv::Theme` (D7). `MenuColors` struct documented (what); the guide's slot-number chain not repeated in the bar's doc. Score 2. |

## Summary

- PORTED: 3   EQUIVALENT: 1   NOT-PORTED: 1   MISSING: 0   UNSURE: 0
- SUSPECT: 0   |   doc<3 (public): 1   |   → concept: 0
- Notable finding: No gaps or suspect items. Both bar-specific methods (`draw`, `get_item_rect`) are fully ported with unit tests and score 3. The only `NOT-PORTED` entry (`Done`) is the expected consequence of Rust's ownership model replacing manual TStreamable/DisposeMenu memory management.
