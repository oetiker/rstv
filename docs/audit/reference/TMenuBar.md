...
| `CMenuView` palette | 479 | EQUIVALENT | OK | `MenuColors::resolve(ctx)` → `Role::Menu*` via `Theme` | 2 | Guide: "Menu bars, like all menu views, use the default palette CMenuView to map onto the 2nd through 7th entries in the standard application palette." Rust: `MenuBar::draw` resolves colours via `MenuColors::resolve` (same six semantic slots). Known idiomatic mapping: class Palette → `tv::Theme` (D7). Deferred to theme pass — the `Role::Menu*` variants live in `src/theme.rs` which is excluded from this sweep. |

## Summary

- PORTED: 3   EQUIVALENT: 1   NOT-PORTED: 1   MISSING: 0   UNSURE: 0
- SUSPECT: 0   |   doc<3 (public): 0   |   → concept: 0
- `MenuBar::new` raised to score 3. Remaining `CMenuView palette` row score 2 deferred to theme pass (lives in `src/theme.rs`). All bar-specific methods (`draw`, `get_item_rect`) were already score 3.
