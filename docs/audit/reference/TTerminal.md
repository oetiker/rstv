...
|---|---|---|---|---|---|---|
| `Init` (constructor) | 553 | PORTED | OK | `tv::Terminal::new(bounds, h_scroll_bar, v_scroll_bar, a_buf_size) -> Terminal` + `Terminal::init(&mut self, ctx)` | 2 | C++ constructor: calls `TTextDevice(bounds, h, v)`, sets `growMode = gfGrowHiX+gfGrowHiY`, caps `bufSize` at 32000, `new`s buffer, then calls `setLimit(0,1)` / `setCursor(0,0)` / `showCursor()` inline. Rust splits this into `new` (geometry + buffer allocation; documented) and `init` (the context-requiring limit/cursor calls; documented). Deviation is commented in both fn docs and module doc. No "why split" guide note needed — module doc is clear. Doc score 2: what each fn does is clear; the "when to call init" pattern could be in an `# Examples` block. |
...
| `queEmpty` (method) | 554 | PORTED | OK | `Terminal::que_empty(&self) -> bool` | 2 | C++: `Boolean queEmpty()` — `queBack == queFront`. Rust: identical. Public. Doc is minimal (one sentence); sufficient. |
