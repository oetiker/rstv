...
| `GetPalette` (method) | 529 | EQUIVALENT | OK | `Role::ScrollerNormal` + `Role::ScrollerSelected` via `ctx.style()` | 2 | → concept: palette chain guide. No public scroller symbol to annotate here; deferred to palette/theme concept pass. |
...
| `CScroller` palette (2 entries) | 530 | EQUIVALENT | OK | `Role::ScrollerNormal` + `Role::ScrollerSelected` in `Theme` | 2 | Roles belong in Theme docs; deferred to theme pass (no public scroller symbol to annotate). |

## Summary

- PORTED: 5   EQUIVALENT: 8   NOT-PORTED: 6   MISSING: 0   UNSURE: 0
- SUSPECT: 0   |   doc<3 (public): 0   |   → concept: 1
- All public symbols raised to score 3. Two rows remain at score 2: `GetPalette` (→ concept: palette chain guide, no scroller symbol to annotate) and `CScroller` palette entries (deferred to theme pass, no scroller symbol to annotate). Notable resolution: `apply_delta` now explicitly identifies itself as the Rust analog of C++ `scrollDraw`.
