...
impl SplitterApp {
    ...
    fn init_desktop(r: Rect) -> Option<Box<dyn View>> {
        ...
        if let Some(v) = win.child_mut(split_id) {
            // grow_mode defaults to { hi_x, hi_y } — the splitter already tracks
            // the window's resize (bottom-right edge follows, top-left stays,
            // keeping the one-cell frame inset).
